...
server.tool(
  ...
  async ({ type, priority, title, description, scope, summary, refs, status }) => {
    try {
...
server.tool(
  ...
  async ({ type }) => text({ tasks: store.getByType(type).map(toListTask) })
);
...
server.tool(
  ...
  async ({ scope }) => text({ scope, tasks: store.getByScope(scope).map(toListTask) })
);
...
server.tool(
  ...
  async ({ status, scope }) => text({ tasks: store.getByStatus(status, scope).map(toListTask) })
);
